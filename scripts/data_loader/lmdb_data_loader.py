...
class SpeechMotionDataset(Dataset):
    def __init__(self, lmdb_dir, n_poses, subdivision_stride, pose_resampling_fps, mean_pose, mean_dir_vec, use_cond = False,
        ...
        preloaded_dir = lmdb_dir + '_cache_beat_audio_combine'
    ...
    def __getitem__(self, idx):
        with self.lmdb_env.begin(write=False) as txn:
            ...
            word_seq, pose_seq, vec_seq, audio, spectrogram, aux_info = pickle.loads(sample)
            if self.use_cond:
                target_vec = utils.data_utils.convert_pose_seq_to_dir_vec(pose_seq,norm=False).reshape(pose_seq.shape[0],-1)
