...
def default_collate_fn(data):
    _, text_padded, pose_seq, vec_seq, audio, spectrogram, aux_info, target_nnm = zip(*data)
    ...
    audio = default_collate(audio)
    spectrogram = default_collate(spectrogram)
    target_nnm = default_collate(target_nnm)
    aux_info = {key: default_collate([d[key] for d in aux_info]) for key in aux_info[0]}

    return torch.tensor([0]), torch.tensor([0]), text_padded, pose_seq, vec_seq, audio, spectrogram, aux_info,target_nnm
...
class SpeechMotionDataset(Dataset):
    def __init__(self, lmdb_dir, n_poses, subdivision_stride, pose_resampling_fps, mean_pose, mean_dir_vec,
                 speaker_model=None, remove_word_timing=False, use_cond=False,use_extra_feature=True):
        ...
        self.use_extra = use_extra_feature

        logging.info("Reading data '{}'...".format(lmdb_dir))
        preloaded_dir = lmdb_dir + '_cache_mel_audio_combine'
    ...
    def __getitem__(self, idx):
        with self.lmdb_env.begin(write=False) as txn:
            ...
            if self.use_cond:
                target_vec = utils.data_utils_expressive.convert_pose_seq_to_dir_vec(pose_seq,norm=False).reshape(pose_seq.shape[0],-1)
        ...
        if do_clipping:
            ...
            if self.use_cond:
                target_vec = target_vec[0:self.n_poses]
        else:
        ...
        # word_seq_tensor = torch.zeros((1,1)).float()
        # extended_word_seq = torch.zeros((1,1)).float()
        ...
        if self.use_cond:
            ...
            return word_seq_tensor, extended_word_seq, pose_seq, vec_seq, audio, spectrogram, aux_info, target_vec
        else:
