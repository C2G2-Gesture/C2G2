...
class DataPreprocessor:
    ...
    def _sample_from_clip(self, vid, clip):
        ...
        print(len(clip_skeleton))
        print(clip_skeleton[0][0])
        ...
        for i in range(num_subdivision):
            ...
            if len(sample_words) >= 2:
                ...
                if (is_correct_motion or self.disable_filtering) and os.path.exists(os.path.join("/home/tiger/nfs/ted_expressive/mediapipe_keypoints",vid+"_")):
                    # print(sample_skeletons[0][2])
                    # print(sample_skeletons[0][3])
                    # print(sample_skeletons[0][4])
                    # print(sample_skeletons[0][5])
                    print(f"vid: {vid}, start_frame: {clip_s_f + start_idx}, end_frame: {clip_s_f + fin_idx}")
                    try:
                        mediapipe_keypoints = self.load_mediapipe(vid,clip_s_f + start_idx,clip_s_f + fin_idx)
                        print(mediapipe_keypoints[0].shape)
                        ...
                        sample_mediapipe_list.append(mediapipe_keypoints)
        ...
        if len(sample_skeletons_list) > 0:
            with self.dst_lmdb_env.begin(write=True) as txn:
                for words, poses, mediapipe_poses, audio, spectrogram, aux in zip(sample_words_list, sample_skeletons_list, sample_mediapipe_list,
                    ...
                    v = [words, poses, normalized_dir_vec, mediapipe_poses, dir_vec_media, audio, spectrogram, aux]
                    v = pickle.dumps(v)
                    txn.put(k, v)
