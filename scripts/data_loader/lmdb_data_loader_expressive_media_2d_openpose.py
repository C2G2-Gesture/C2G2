...
from data_loader.data_preprocessor_expressive_2d_openpose import DataPreprocessor
...
def default_collate_fn(data):
    pose_seq, vec_seq, audio, spectrogram, aux_info = zip(*data)
    ...
    return pose_seq, vec_seq, audio, spectrogram, aux_info
...
class SpeechMotionDataset(Dataset):
    def __init__(self, lmdb_dir, n_poses, subdivision_stride, pose_resampling_fps, mean_pose, mean_dir_vec,
        ...
        preloaded_dir = lmdb_dir + '_cache11_2d_openpose_v3'
    ...
    def __getitem__(self, idx):
        with self.lmdb_env.begin(write=False) as txn:
            ...
            pose_seq, vec_seq, audio, spectrogram, aux_info = sample
            # if self.use_cond:
            #     target_vec = utils.data_utils_expressive.convert_pose_seq_to_dir_vec_2d(pose_seq,norm=False).reshape(pose_seq.shape[0],-1)
