...
from data_loader.data_preprocessor_expressive_2d import DataPreprocessor
...
class SpeechMotionDataset(Dataset):
    def __init__(self, lmdb_dir, n_poses, subdivision_stride, pose_resampling_fps, mean_pose, mean_dir_vec,
        ...
        # preloaded_dir = lmdb_dir + '_cache_with_hands'
        preloaded_dir = lmdb_dir + '_cache11_2d'
    ...
    def __getitem__(self, idx):
        with self.lmdb_env.begin(write=False) as txn:
            ...
            word_seq, pose_seq, vec_seq, media_pose, media_vec, audio, spectrogram, aux_info = sample
        ...
        if do_clipping:
            ...
            media_vec_seq = media_vec[0:self.n_poses]
            media_pose_seq = media_pose[0:self.n_poses]
        else:
        ...
        media_vec_seq = torch.from_numpy(copy.copy(media_vec_seq)).reshape((media_vec_seq.shape[0], -1)).float()
        media_pose_seq = torch.from_numpy(copy.copy(media_pose_seq)).reshape((media_pose_seq.shape[0], -1)).float()
        audio = torch.from_numpy(copy.copy(audio)).float()
        spectrogram = torch.from_numpy(copy.copy(spectrogram)).float()

        return word_seq_tensor, extended_word_seq, media_pose_seq, media_vec_seq, audio, spectrogram, aux_info
