...
def train_epochs(args, train_data_loader, lang_model, pose_dim, speaker_model=None, use_cond=True, weight_path=None):
    start = time.time()
    ...
    if mode == "vqvae":
        if use_cond:
            vqvae = CondVQModel(in_channels=126,n_embed=1024,embed_dim=128,hidden_channels=128,num_res_blocks=2,deeper=False,pos_enc=False).to(device)
            ...
            vqvae = VQModel(in_channels=126,n_embed=1600,embed_dim=512,hidden_channels=256,num_res_blocks=2,deeper=False,pos_enc=False).to(device)
...
def main(config):
    ...
    train_epochs(args, train_loader, lang_model,
                 pose_dim=pose_dim, speaker_model=train_dataset.speaker_model,weight_path=args.vqvae_weight)
