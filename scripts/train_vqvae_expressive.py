...
def train_epochs(args, train_data_loader, lang_model, pose_dim, val_loader,speaker_model=None):
    ...
    if mode == "vqvae":
        # vqvae = VQModel(in_channels=126,n_embed=1024,embed_dim=128,hidden_channels=128,num_res_blocks=2,deeper=False,pos_enc=False).to(device)
        vqvae = VQModel(in_channels=126,n_embed=512,embed_dim=64,hidden_channels=128,num_res_blocks=2,deeper=False,pos_enc=False).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        for iter_idx, data in enumerate(tqdm(train_data_loader)):
            ...
            if mode == "vqvae":
                loss = train_iter_vqvae(args, target_vec, 
...
def eval(vqvae,val_loader,args,lang_model,device):
    ...
    with torch.no_grad():
        ...
        for iter_idx, data in enumerate(tqdm(val_loader)):
            _, in_text_padded, _, _, target_vec, in_audio, _, _ = data

            batch_size = target_vec.size(0)
            in_audio = in_audio.to(device)
            target_vec = target_vec.to(device)
            
            out_dir_vec,diff = vqvae(target_vec)
            embed_space_evaluator.push_samples(in_text_padded, in_audio, out_dir_vec, target_vec)
