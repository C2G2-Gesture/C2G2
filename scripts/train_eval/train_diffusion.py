...
def train_iter_diffusion(args, in_audio, target_poses, pose_diffusion, optimizer, in_mel = None):
    ...
    loss,recon_loss = pose_diffusion.get_loss(target_poses, pre_seq, in_audio,in_mel)
...
def eval_iter_diffusion(args, in_audio, target_poses, pose_diffusion, optimizer):
...
def train_iter_vqvae_cond(args, target_poses, pose_vqvae, optimizer,nnm_target):
    optimizer_g, optimizer_d = optimizer
    ...
    loss,rec_loss,quant_loss = pose_vqvae.training_step(target_poses,nnm_target,0,0)
    total_loss = loss
    total_loss.backward()
...
def train_iter_vae(args, target_poses, pose_vqvae, optimizer):
    ...
    loss,kl = pose_vqvae.training_step(target_poses,0,0)
    total_loss = loss
    total_loss.backward()

    optimizer_g.step()

    ret_dict = {'loss':loss.item(),"kl_loss":kl.item()}
    return ret_dict
