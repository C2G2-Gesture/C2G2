...
from model.pose_diffusion import PoseLatentDiffusion
from parse_args_diffusion import parse_args
from train_eval.train_diffusion import train_iter_diffusion,eval_iter_diffusion
...
def train_epochs(args, train_data_loader, lang_model, pose_dim, speaker_model=None):
    ...
    loss_meters = [AverageMeter('loss'), AverageMeter('var_loss'), AverageMeter('gen'), AverageMeter('dis'),
                   AverageMeter('KLD'), AverageMeter('DIV_REG'),AverageMeter('recon_loss')]
...
def eval(eval_data_loader):
    for iter_idx, data in enumerate(tqdm(eval_data_loader)):
        _, _, _, _, target_vec, in_audio, _, _ = data
        ...
        if args.model == 'pose_diffusion':
            loss = eval_iter_diffusion(args, in_audio, target_vec,
                                    diffusion_model, optimizer)
