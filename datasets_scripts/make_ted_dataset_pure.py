...
def make_ted_gesture_dataset():
    ...
    sample_path,exposes_path = get_all_clips()
    # video_files = video_files[:10]
    
    image_datas = os.listdir(sample_path)
    for sample in image_datas:
        images_path = os.path.join(sample_path,sample,"images")
        openpose_path = os.path.join(sample_path,sample,"keypoints")
        expose_path = os.path.join(exposes_path,sample)
...
def get_all_clips(datapath = '/home/tiger/nfs/Jessica_cropped_v1'):
    
    expose_path = os.path.join(datapath,"expose_ted")
    image_path = os.path.join(datapath,"temp_skeleton_raw")
